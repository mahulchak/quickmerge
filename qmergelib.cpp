...
map<int,int> ovl2qEnd;
	if(q_st.size() >1)
	{
			...
			if(q_st[j]<q_end[j]) //if this MUM is forward oriented
			{
				...
				{
					if((q_st[k] > (storedQend - 500)) && (abs(q_st[k]-storedQend)<merge.q_len[tempname]) && (ref_st[k] > storedRefEnd)) //500bp overlap allowed
					{
			...
			if(q_st[j]>q_end[j]) //if this MUM is reverse oriented
			{
				...
				for(unsigned k = j;k<q_st.size();k++)
				{
					...
					if(k>j)
					{
						if((q_st[k] < (storedQend+500)) && (abs(q_st[k]-storedQend)<merge.q_len[tempname]) && (ref_st[k] > storedRefEnd))
						{
							ovl = ovl + abs(q_st[k] - q_end[k]);
                                                        storedQend = q_end[k];
							storedRefEnd = ref_end[k];
